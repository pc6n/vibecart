...
/**
 * Utility class for binary serialization/deserialization using MessagePack
 */
export class BinarySerializer {
    ...
    static serialize(data) {
        if (!data) {
            console.error('[BinarySerializer] Attempted to serialize null or undefined data');
     ...
     * @param {Uint8Array|ArrayBuffer|Object|string} binaryData - The data to deserialize
     * @returns {Object} - Deserialized JavaScript object
     */
    static deserialize(binaryData) {
        if (binaryData === null || binaryData === undefined) {
            console.error('[BinarySerializer] Attempted to deserialize null or undefined data');
            return {}; // Return empty object instead of throwing error
        }
        
        try {
            // If the input is already an object (but not binary), just return it
            if (typeof binaryData === 'object' && !(binaryData instanceof Uint8Array) && !(binaryData instanceof ArrayBuffer)) {
            ...
            if (typeof binaryData === 'string') {
                try {
                    ...
                    console.warn('[BinarySerializer] Failed to parse string as JSON, attempting msgpack decode');
            ...
            console.error('[BinarySerializer] Error deserializing data:', error);
